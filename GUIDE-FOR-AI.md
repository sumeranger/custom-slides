...
| token | 用途 | 值 |
|------|------|----|
| `--t-display-1` | 滿版巨大數字／單字 | clamp(140, 200) |
| `--t-display-2` | （次）巨大數字 | clamp(90, 128) |
| `--t-h1` | 標題頁 · 章節開場主題大標 | clamp(64, 96) |
| `--t-h2` | 每步頁面主標 `<h2>` | clamp(48, 64) |
| `--t-h3` | 副標 / 大卡片標題 | 40 |
| `--t-lead` | deck / 副標句 / 領句 | 30 |
| `--t-body` | 內文 / 描述 | 26 |
...
1. **眉題**（小、accent 色）：段落軸標記，如 `坑 1` / `第 2 章` / `Part 3`。
   是報告的骨架，要醒目——給 `~44px`（大於 `--t-label`，小於主題大標）。
